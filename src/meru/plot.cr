  ...
  module Plot
    ...
    module Unicode
      ...
      private def smudge_matrix(
        ...
        matrix.map do |row|
          row.map { |value| value > 0.0 ? Math.log10(value + 1.0) : Float64::NAN }
        end
      end
    end
    ...
    module Text
      ...
      private def density_index(value : UInt64, max_value : UInt64, palette_size : Int32) : Int32
        return 0 if value == 0_u64 || max_value == 0_u64
        scaled = ((value.to_f / max_value.to_f) * (palette_size - 1)).ceil.to_i
        scaled < 1 ? 1 : scaled
      end
    end
  end
